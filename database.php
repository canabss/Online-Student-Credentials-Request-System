<?php
	...
	function isStudent($studentNo, $birthday, $course){
		...
		while($data = mysqli_fetch_assoc($sql)){
			if($studentNo == $data['student_no'] && $birthday == $data['birthday'] && $course == $data['course']){
				$isExist = true;
				break;
			}
		}
	...
	function submitRequestAlumni($studentNo, $email, $lastName, $firstName, $middleName, $documents, $year, $semester, $purpose, $requestor, $needClearance, $file){
		$db = database();
		$add = null;
		$dateTimeZone = new DateTime("now", new DateTimeZone('Asia/Manila'));
		$date = $dateTimeZone->format('Y-m-d');
		...
		if($clearanceResult){
			...
			if($haveClearance){
				...
				$myClearanceId = $dateTimeZone->format('Y')."-".($clearanceRow['rows']+1);
				if(empty($file)){
				    $sql = $db->query("INSERT INTO clearance VALUES('$myClearanceId', '$studentNo', '$email', '$year', '$semester', 'false', 'false', 'false', 'false', 'false', 'false', 'false', 'false', 'false', 'To Confirm', '$file')");
				}
				else{
			    	$sql = $db->query("INSERT INTO clearance VALUES('$myClearanceId', '$studentNo', '$email', '$year', '$semester', 'true', 'true', 'true', 'true', 'true', 'true', 'true', 'true', 'true', 'Complete', '$file')");
				}
	...
	function getClearanceSignatories($status){
		$db = database();
		$personnels = [];
		if(!($db -> connect_error)){
			$sql = $db->query("SELECT * FROM personnels WHERE status = '$status' AND NOT role = '1'");
	        while($data = mysqli_fetch_assoc($sql)){
	            $personnels[] = $data;
	        }
	    }
