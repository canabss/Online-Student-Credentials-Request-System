<?php
    ...
    function lastnameValidation($lastName, $error){
        if(!preg_match("/^[a-zA-Z-' ]*$/", $lastName)) {
            $error[] = 'Special characters and/or numbers are not allowed in Lastname.';
    ...
    function firstnameValidation($firstName, $error){
        if(!preg_match("/^[a-zA-Z-' ]*$/", $firstName)) {
            $error[] = 'Special characters and/or numbers are not allowed in Firstname.';
    ...
    function middlenameValidation($middleName, $error){
        if(!preg_match("/^[a-zA-Z-' ]*$/", $middleName)){
            $error[] = 'Special characters and/or numbers are not allowed in Middlename.';
    ...
    function documentValidation($documents, $error){
        if(empty($documents)){
            $error[] = 'No document(s) is/are selected.';
            $_SESSION['docs'] = $documents;
        }
        return $error;
    }
    ...
    function specifySessionSet($purpose){
        if(!empty($purpose)) {
            $_SESSION['specify-purpose'] = $purpose;
        }
    }
